...
\nВо-первых, всегда полезно знать о практическом применении современных информационных технологиях. Во-вторых, в ходе выполнения работы вы сможете изучить этапы разработки решения с применением искусственного интеллекта. 
...
st.write("""Здесь мы свами впервые встречаемся с таким понятием как логические операторы.""")
st.write("""
...
task0 = st.selectbox("Что такое первичный ключ", ["", "Самый главный столбец, который содержит уникальный идентификатор для каждого объекта",
                                                  "Столбец, который содержит id пользователей",
...
task1 = st.selectbox("Выберете верную последовательности выполнения операторов в SQL", ["", "SELECT, FROM, WHERE, ORDER BY", "FROM, WHERE, SELECT, ORDER BY", "SELECT, ORDER BY, WHERE, FROM"])

if task1 == "FROM, WHERE, SELECT, ORDER BY":
    right_answers += 1

task2 = st.selectbox("Выберете верную последовательности выполнения логических операторов в SQL", ["", "AND, NOT, OR", "NOT, AND, OR", "Операторы выполняются последовательно слева направо"])

if task2 == "NOT, AND, OR":
    right_answers += 1

task3 = st.selectbox("Для чего используется оператор ORDER BY", ["", "Для сортировки данных удовлетворяющих определенному условию",
                                                                 "Для поиска записи по ключу",
                                                                 "Для сортировки данных в порядке возрастания (ASC) или убывания (DESC)"])

if task3 == "Для сортировки данных в порядке возрастания (ASC) или убывания (DESC)":
    right_answers += 1

st.write("""\n Выполнится ли следующий запрос:""")
...
st.write("""\n Что вернет следующий запрос к таблице department:""")
st.write("""**SELECT AVG(Зарплата) FROM department GROUP BY Пол HAVING Возраст >= 30**""")
st.image("src/department.png")
task5 = st.selectbox("", ["",
                          "Среднюю зарплату сгруппированную по полу для лиц больше 30 лет",
                          "Среднюю зарплату для лиц больше 30 лет",
...
st.write("""\n Что вернет следующий запрос к таблице report:""")
...
first_query = st.text_input("""Введите SQL-запрос""")
...
if first_query != "":
    try:
        ...
        if (result.values == answer.values).all():
...
second_query = st.text_input("""Введите  SQL-запрос""")
...
if second_query != "":
    try:
        ...
        columns = [st.strip() for st in columns]
        result = pd.DataFrame(result, columns=columns)
        if (result.values == answer_hd.values).all():
            st.write("Запрос написан верно", result)
            right_answers += 1
        else:
            st.write('Результат выполнения вашего запроса', result)
